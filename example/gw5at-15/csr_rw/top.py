...
Prerequisites:
    - serdes.csr in the same directory (generated by Gowin IDE or
      serdes_toml_gen.py + serdes_toml_to_csr tool)
"""
...
def build(do_program=False):
    ...
    # Add the SerDes CSR blob (must match the LaneConfig / data rates).
    csr_path = Path(__file__).parent / "serdes.csr"
    if not csr_path.exists():
        # Fallback: check build directory from previous run
        csr_path = Path(__file__).parent / "build" / "serdes.csr"
    if not csr_path.exists():
        raise FileNotFoundError(
            "serdes.csr not found. Generate it with serdes_toml_gen.py "
            "and the Gowin serdes_toml_to_csr tool, then place it next "
            "to top.py."
        )
