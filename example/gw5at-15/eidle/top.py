...
The 125 MHz board oscillator is enabled (clk_en=1) as the SerDes PLL
reference clock but is NOT used for fabric logic.
...
GowinSerDes configuration via ``serdes.generate_csr()``.
If the Gowin IDE tools are not installed, place a pre-generated
``serdes.csr`` next to this file.
"""
...
class SerDesDRPBridge(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        # Enable the 125 MHz oscillator (SerDes PLL reference clock).
        m.d.comb += clk_en.o.eq(1)
