...
class SerDesDRPBridge(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        # -- Drive reset (PMA in reset, PCS in reset) --
        m.d.comb += [
            lane0.reset.pma_rstn.eq(0),  # PMA held in reset (active-low)
            lane0.reset.pcs_rx_rst.eq(1),  # PCS RX held in reset (active-high)
            lane0.reset.pcs_tx_rst.eq(1),  # PCS TX held in reset (active-high)
        ]
...
def build(do_program=False):
    ...
    # Generate the SerDes CSR directly from the design's configuration.
    lane_cfg = LaneConfig(
        operation_mode=OperationMode.TX_RX,
        tx_data_rate="1.25G",
        rx_data_rate="1.25G",
