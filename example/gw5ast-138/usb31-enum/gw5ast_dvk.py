...
class GW5ASTDVKPlatform(GowinPlatform):
    ...
    resources = [
        ...
        # Legacy UART via BL616 debugger on SOM (not reliably forwarded to USB)
        # TX: P15 (FPGA → BL616), RX: N16 (BL616 → FPGA)
        Resource(
            "uart",
            1,
            Subsignal("tx", Pins("P15", dir="o")),
            Subsignal("rx", Pins("N16", dir="i")),
            Attrs(IO_TYPE="LVCMOS33"),
        ),
    ]
    ...
    def create_missing_domain(self, name):
        ...
        PLL and the SerDes recovered clock.  Returning ``None`` prevents
        Amaranth from auto-creating a ``sync`` domain or any other
        missing domain.
        """
        return None
    ...
    def toolchain_prepare(self, fragment, name, **kwargs):
        ...
        **Raw mode** (``name == "raw_serdes_sniffer"``):
        ...
        if is_usb31_enum:
            # USB3.1 enumeration example.  The Gowin PnR auto-detects the
            # SerDes hard-macro clocks and names the clock objects after
            # the nets feeding the PHY; constrain those nets with the REAL
            # frequencies (auto-detection defaults to a too-weak 100 MHz):
            #   pclk / rxclk : 156.25 MHz in the Gen2 boot trim (6.4 ns),
            #                  125 MHz after Gen1 fallback -- constrain 6.4.
            #   upar life clk: 62.5 MHz (16 ns).
            # All fabric CDC crossings use AsyncFIFO / FFSynchronizer.
            sdc_constraints = [
                ...
                "create_clock -name upar_clk -period 16.0 "
                "[get_nets {serdes_upar_clk_i}]",
                "",
                "set_false_path -from [get_clocks {clk50_0__io}] "
                "-to [get_clocks {pclk rxclk upar_clk}]",
                "set_false_path -from [get_clocks {pclk rxclk upar_clk}] "
                "-to [get_clocks {clk50_0__io}]",
                "set_false_path -from [get_clocks {pclk}] "
                "-to [get_clocks {rxclk upar_clk}]",
                "set_false_path -from [get_clocks {rxclk}] "
                "-to [get_clocks {pclk upar_clk}]",
                "set_false_path -from [get_clocks {upar_clk}] "
                "-to [get_clocks {pclk rxclk}]",
            ]
        elif is_link_training:
